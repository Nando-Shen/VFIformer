...
class TFL(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if not use_crossattn:
            ...
            self.attn = WindowCrossAttention(
                dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
            # self.strid_conv = nn.Conv2d(dim, dim, 4, 2, 1)
    ...
    def calculate_mask2(self, x_size):
        ...
        mask_windows_down = mask_windows_down.view(self.window_size*self.window_size, -1).permute(1, 0).contiguous()  # nW, window_size*window_size

        attn_mask = mask_windows_down.unsqueeze(1) - mask_windows.unsqueeze(2)  # nW, window_size*window_size, window_size*window_size
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
    ...
    def forward(self, x, x_size):
        ...
        if not self.use_crossattn:
            ...
            x_windows_down = x_windows_down.permute(0, 3, 2, 1).contiguous().view(-1, self.window_size*self.window_size, C)  # nW*B, window_size*window_size, C
...
class TFModel(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3, out_chans=3, fuse_c=16,
        ...
        # stochastic depth
        dpr0 = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[0]))]  # stochastic depth decay rule
        dpr1 = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[1]))]  # stochastic depth decay rule
        dpr2 = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[2]))]  # stochastic depth decay rule
        dpr3 = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[3]))]  # stochastic depth decay rule
        ...
        for i_layer in range(num_layers):
            layer = RTFL(dim=embed_dim,
                         ...
                         drop_path=dpr0[sum(depths[0][:i_layer]):sum(depths[0][:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
        ...
        for i_layer in range(num_layers):
            layer = RTFL(dim=embed_dim,
                         ...
                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop_rate, attn_drop=attn_drop_rate,
                         drop_path=dpr1[sum(depths[1][:i_layer]):sum(depths[1][:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
        ...
        for i_layer in range(num_layers):
            layer = RTFL(dim=embed_dim,
                         ...
                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop_rate, attn_drop=attn_drop_rate,
                         drop_path=dpr2[sum(depths[2][:i_layer]):sum(depths[2][:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
        ...
        for i_layer in range(num_layers):
            layer = RTFL(dim=embed_dim,
                         ...
                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop_rate, attn_drop=attn_drop_rate,
                         drop_path=dpr2[sum(depths[3][:i_layer]):sum(depths[3][:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
        ...
        self.conv_after_body0 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim, 3, 2, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                              nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        self.conv_after_body1 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim, 3, 2, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                              nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        self.conv_after_body2 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim, 3, 2, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                              nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),)
